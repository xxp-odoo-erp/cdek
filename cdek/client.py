...
class CdekClient:
        ...
        try:
            if method == "GET":
                response = self._session.get(
                    f"{self.base_url}{url}", params=request_params, headers=headers, timeout=self.timeout
                )
            elif method == "POST":
                response = self._session.post(
                    f"{self.base_url}{url}", json=request_params, headers=headers, timeout=self.timeout
                )
            elif method == "PATCH":
                ...
                response = self._session.delete(f"{self.base_url}{url}", headers=headers, timeout=self.timeout)
            else:
    ...
    def _authorize(self) -> bool:
        ...
        try:
            response = self._session.post(
                f"{self.base_url}{constants.OAUTH_URL}", data=params, headers=headers, timeout=self.timeout
            )
    ...
    def get_city_zip_codes(self, city_code: str) -> list[str]:
        response = self._api_request("GET", constants.ZIP_CODES_URL, {"city_code": city_code})
        return response.get("postal_codes", [])
