...
class CityFilter(BaseRequest):
    ...
    country_code: str | None = Field(None, description="Код страны в формате ISO_3166-1_alpha-2")


class CityListFilter(BaseRequest):
    """Фильтр для списка городов"""

    country_codes: str | None = Field(None, description="Массив кодов стран в формате ISO_3166-1_alpha-2")
    ...
    city: str | None = Field(None, description="Название населенного пункта. Должно соответствовать полностью")
    payment_limit: float | None = Field(None, description="Ограничение на сумму наложенного платежа")
    size: int | None = Field(1000, description="Ограничение выборки результата")
    page: int | None = Field(0, description="Номер страницы выборки результата. Нумерация страниц с 0")
...
class RegionFilter(BaseRequest):
    ...
    country_codes: str | None = Field(None, description="Список кодов стран в формате ISO_3166-1_alpha-2")
...
class ZipFilter(BaseRequest):
    ...
    city_code: int = Field(..., description="Код города, которому принадлежат почтовые индексы")
