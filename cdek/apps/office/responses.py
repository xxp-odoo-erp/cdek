...
class OfficeResponse(BaseModel):
    ...
    nearest_station: str | None = Field(None, description="Ближайшая станция/остановка транспорта")
    nearest_metro_station: str | None = Field(None, description="Ближайшая станция метро")
    work_time: str = Field(..., description="Режим работы, строка вида «пн-пт 9-18, сб 9-16»")
    ...
    type: str = Field(..., description="Тип ПВЗ. PVZ — склад СДЭК, POSTAMAT — постамат СДЭК")
    owner_code: str = Field(..., description="Принадлежность офиса компании")
    take_only: bool = Field(
        ..., description="Является ли офис только пунктом выдачи или также осуществляет приём грузов"
    )
    ...
    is_marketplace: bool | None = Field(None, description='Офис для доставки заказов "До маркетплейса"')
    is_ltl: bool | None = Field(None, description="Работает ли офис с LTL (сборный груз)")
    have_cashless: bool = Field(..., description="Есть безналичный расчет")
    have_cash: bool = Field(..., description="Есть приём наличных")
    have_fast_payment_system: bool = Field(..., description="Есть безналичный расчёт по СБП")
    ...
    office_image_list: list[Image] | None = Field(None, description="Все фото офиса (кроме фото как доехать)")
    work_time_list: list[WorkTime] = Field(..., description="График работы на неделю")
    work_time_exception_list: list[WorkTimeException] = Field(..., description="Исключения из графика работы")
    weight_min: float | None = Field(None, description="Минимальный вес (в кг.), принимаемый в ПВЗ (> WeightMin)")
    weight_max: float | None = Field(None, description="Максимальный вес (в кг.), принимаемый в ПВЗ (<= WeightMax)")
    ...
    distance: int | None = Field(None, description="Расстояние до точки, от которой производился поиск ближайшего ПВЗ")
    fulfillment: bool | None = Field(None, description='Работает ли офис с "Фулфилмент. Приход"')
